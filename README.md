...
- **自动点击订阅默认关闭**：脚本加载后只观察页面、识别状态和提醒，不会主动点击订阅进入购买链路。我们的担忧是：正常用户在开售前通常无法从页面主动触发购买/确认请求，如果脚本一加载就把按钮激活并自动点击，可能在错误时间打到正常用户按不到的关键接口，从而被风控标记。需要用户在配置面板开启，或按 `F9` 显式开启；如果启用了 Rush mode，目标时间已到后才允许自动点击订阅。
...
- 默认会自动识别验证码并点击目标文字。
- 默认不会自动点击订阅按钮，避免脚本加载后主动触发购买链路。这样做是为了降低“开售前请求正常用户按不到的关键接口”带来的风控风险；需要在配置面板或用 `F9` 显式开启。
...
默认配置比较保守：脚本会帮你识别并点选验证码文字，但不会自动点击订阅，也不会替你按验证码“确定”。自动点击订阅需要你手动开启或等 Rush 目标时间到达。
